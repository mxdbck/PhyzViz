...
fn setup(mut commands: Commands, mut effects: ResMut<Assets<EffectAsset>>) {
    ...
    spawn_ribbon_emitter(&mut commands, &mut effects, "pendulum_ribbon".to_string(), &RibbonParams::default());
}
...
fn draw_pendulum(mut painter: ShapePainter, state: Res<PendulumState>, mut q: Query<&mut Transform, With<ParticleEffect>>) {
    ...
    if let Ok(mut transform) = q.single_mut() {
        let bob_pos = bob_pos * RENDER_SCALE;
